...
template <typename TChar> struct BasicStringPiece {
  const TChar *data_;
  size_t length_;
};
...
public:
  ...
  inline static auto stringAtS_ =
      ("_ZNK7android13ResStringPool8stringAtEjPj"_sym |
       "_ZNK7android13ResStringPool8stringAtEmPm"_sym)
          .as<stringAtRet (ResStringPool::*)(size_t)>;

  inline static auto stringAt_ =
      ("_ZNK7android13ResStringPool8stringAtEj"_sym |
  ...
  StringPiece16 stringAt(size_t idx) const {
    if (stringAt_) {
      size_t len;
      const char16_t *str =
          stringAt_(const_cast<ResStringPool *>(this), idx, &len);
      return {str, len};
    } else if (stringAtS_) {
